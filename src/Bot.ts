import { Bot, webhookCallback, CommandContext, Context, InputFile } from "grammy";
 ...
 */
async function initBot(cusConfig: CusConfig, isProxy: boolean) {
    ...
    if (isProxy) {
        ...
        bot = new Bot(cusConfig.BotToken, {
            client: {
                baseFetchConfig: {
                    agent: socksAgent,
                    compress: true,
    ...
    for (const kind of cusConfig.GetTagsKind()) {
        commandsMap.set(`\`/${kind}\``, { help: `random ${formatMarkDownStr(kind)} tags image` });
        bot.command(kind, imgCBGenerate(kind));
    }
    commandsMap.set('\`/set_rating\`', { help: 'set the image rating' });
    commandsMap.set('\`/tag <image_tags>\`', { help: 'get image through your custom tag' });
    commandsMap.set(`\`/id <image_id>\``, { help: 'get image through the image id' });
    ...
    bot.on('callback_query:data', async (ctx) => {
        ...
        switch (callbackData) {
            ...
            case 'explicit':
                ...
                await ctx.answerCallbackQuery(`The Rating has been set: ${callbackData}`);
                // 处理 Option 4 的逻辑
                break;
            case 'disable':
                dan.DisableRating();
 ...
 * @return {*}
 */
async function startBotDevMode(cusConfig: CusConfig) {

    const bot = await initBot(cusConfig, true);
