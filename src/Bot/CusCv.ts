...
class CusCvClass {
    ...
    async AddTags(conversation: CusConversation, ctx: CusContext) {
        const nowTags = await this._ucMan.GetKindTags(
            ctx.chat!.id,
            ctx.session.tagKind,
        );
        await ctx.reply(
            `[Kind]: ${ctx.session.tagKind}\n[tags]: ${nowTags.join(",")}\n Input the tag you want to add, divided by space\nOr you can just enter ${ReserveWord.exit} to exit.`,
        );
        ...
        ctx.reply(
            `Add ${actionRet.length} tags: ${actionRet} to kind: ${ctx.session.tagKind} successfully.\nnew tag count: ${actionRet.length}`,
        );
    }
    ...
    async RmTags(conversation: CusConversation, ctx: CusContext) {
        ...
        ctx.reply(
            `Remove ${actionRet.length} tags: ${actionRet} from kind: ${ctx.session.tagKind} successfully.`,
        );
    }
