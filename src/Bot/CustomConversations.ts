...
function initAllConversation(bot: Bot<CusContext>) {
    bot.use(createConversation(
        async (conversation: CusConversation, ctx: CusContext) => {
    ...
    bot.use(
        createConversation(async (conversation: CusConversation, ctx: CusContext) => {
            const kind = ctx.session.tagKind
            const { id: kindId } = await conversation.external(() => sql.SelectKindIdCount(ctx.chat!.id, kind));
            await ctx.replyFmt(
...
export { cvNames, initAllConversation };
